...
void Controller::startup()
{
    ...
    *connPtr2 = connect(this, &Controller::upToDate, this, [this, connPtr2] {
        const auto optTxId = storage->txidForTxNum(1, false, nullptr, true); // skip cache, read from txNum2txId file to get first non-genesis tx hash
        if (!optTxId) return; // no blocks yet, somehow. must be a brand new chain. we will be called again later when blocks start showing up.
        auto onSuccess = [this, connPtr2, txid=*optTxId] (const RPC::Message &resp) {
            ...
            try {
                ...
                Error() << "Failed to deserialize tx #1 with txid " << txid.toHex() << ": " << e.what();
                Error() << kErrLine2;
                return;
            }
            if (BTC::Hash2ByteArrayRev(tx.GetId()) != txid) {
                Error() << "Failed to validate tx #1 with txid " << txid.toHex();
                Error() << kErrLine2;
                return;
            }
        ...
        bitcoindmgr->submitRequest(this, IdMixin::newId(), "getrawtransaction", {Util::ToHexFast(*optTxId), false}, onSuccess, onError, onFail);
    }, Qt::QueuedConnection);
    ...
    submitRequest("getblockhash", {bnum}, [this, bnum](const RPC::Message & resp){
        ...
        submitRequest("getblock", {var, false}, [this, bnum, hash](const RPC::Message & resp){
            try {
                ...
                if(chkHash != hash) {
                    Warning() << resp.method << ": at height " << bnum << " header not valid (expected hash: " << chkHash.toHex() << ", got hash: " << hash.toHex() << ")";
                    errorCode = int(bnum);
                    errorMessage = QString("hash mismatch for height %1").arg(bnum);
                    emit errored();
                }

                PreProcessedBlockPtr ppb = PreProcessedBlock::makeShared(bnum, size_t(rawblock.size()), block);
                assert(bool(ppb));
...
struct SynchMempoolTask : public CtlTask
{
    ...
    std::unordered_set<TxId, HashHasher> txsFailedDownload; ///< set of tx's dropped due to RBF and/or mempool pressure as we were downloading
    ...
    Mempool::TxIdSet dspTxsAffected;
    /// The txids either added or dropped -- for the txSubsMgr
    std::unordered_set<TxId, HashHasher> txidsAffected;
    ...
    Tic cachet0;
    {
        ...
        for (const auto & [txid, pair] : txsDownloaded) {
            ...
            for (const auto & in : ctx->vin) {
                ...
                if (mempool.txs.count(txo.txId) || txsDownloaded.count(txo.txId))
...
void SynchMempoolTask::doGetRawMempool()
{
    submitRequest("getrawmempool", {false}, [this](const RPC::Message & resp){
        ...
        Mempool::TxIdSet droppedTxs;
        {
            ...
            auto [mempool, lock] = storage->mempool();
            droppedTxs = Util::keySet<Mempool::TxIdSet>(mempool.txs);
        }
        for (const auto & var : txidList) {
            const auto txidHex = var.toString().trimmed().toLower();
            const TxId hash = Util::ParseHexFast(txidHex.toUtf8());
            if (hash.length() != HashLen) {
...
auto Controller::stats() const -> Stats
{
    ...
    m["TxNum"] = txnum;
    m["TxNum -> TxId (latest)"] = txnum ? storage->txidForTxNum(TxNum(txnum-1), false, nullptr, true).value_or("").toHex() : QVariant();
    if (sm) {
        QVariantMap m2;
...
auto Controller::debug(const StatsParams &p) const -> Stats // from StatsMixin
{
    ...
    if (const auto txnum = p.value("txnum").toULong(&ok); ok) {
        QVariantMap m;
        auto txid = storage->txidForTxNum(txnum).value_or(QByteArray());
        auto opt = storage->heightForTxNum(txnum);
        m["tx_hash"] = Util::ToHexFast(txid);
